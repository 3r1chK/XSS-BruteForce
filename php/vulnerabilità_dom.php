<?php
 ...
 if (isset($_POST['urlXSS'])) {
   ...
   $url_nopath = $url_array['scheme']."://".$url_array['host']."/";
       ...
       if (!empty($dati_testo)) {
         ...
         foreach ($dati_testo as $dato_testo => $value) {
           if ($attacco_riuscito_post == false) {
             ...
             echo $dati_post_string."\r\n";
             curl_setopt($ch, CURLOPT_URL, $url_post);
             curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
             curl_setopt($ch, CURLOPT_POST, true);
             curl_setopt($ch, CURLOPT_POSTFIELDS, $dati_p);
             curl_setopt($ch, CURLOPT_COOKIEFILE, $cookie_file);
             curl_setopt($ch, CURLOPT_COOKIEJAR, $cookie_file);
             curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
           ...
           if ($attacco_riuscito_post == false){
             echo "Form : ".$n_formPOST." --> POST : Tentativo di attacco fallito con ".$script2."\r\n";
           }
         }
         //}
   ...
   if (($html->find('form[method=get],form[method=GET],form[!method]'))!=NULL) {
     foreach ($html->find('form[method=get],form[method=GET],form[!method]') as $forms_get) {
       $n_formGET++;
       $formGET_action = $forms_get->action;
       ...
       if (!empty($dati_testo_get)) {
         ...
         foreach ($dati_testo_get as $dato_testo_get => $value) {
           ...
           if ($attacco_riuscito_get == false) {
             $dati_g = array_merge($input_script_get, $dati_random_get);
             foreach($dati_g as $key => $value){
               $dati_get[] = $key."=".$value;
             }
             $dati_get_string = implode ('&', $dati_get);

             //Creazione dell'url per la GET
             $url_get = $url_nopath.$formGET_action."?".$dati_get_string;
           ...
           if ($attacco_riuscito_post == false){
             echo "Form : ".$n_formGET." --> GET : Tentativo di attacco fallito con ".$script2."\r\n";
           }
